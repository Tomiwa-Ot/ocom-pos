        ...
        ThreadPoolManager.getInstance().executeTask(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    byte[] sign1 = "****************".getBytes("gb2312");
                    byte[] fontTest0 = "这是一行24号字体\n".getBytes("gb2312");
                    byte[] fontTest1 = "这是一行24号倍高字体\n".getBytes("gb2312");
                    byte[] fontTest2 = "这是一行24号倍宽字体\n".getBytes("gb2312");
                    byte[] fontTest3 = "这是一行48号字体\n".getBytes("gb2312");
                    byte[] orderSerinum = "1234567890\n".getBytes("gb2312");
                    byte[] specialSign= "!@#$%^&*(κρχκμνκλρκνκνμρτυφ)\n".getBytes("gb2312");
                    byte[] testSign = "----------------".getBytes("gb2312");
                    byte[] testInfo = "欢迎使用蓝牙打印机\n".getBytes("gb2312");
                    byte[] nextLine = ESCUtil.nextLines(1);
                    byte[] performPrint = ESCUtil.performPrintAndFeedPaper((byte)200);

                    byte[][] cmdBytes = {printer_init,selectChinese,charCode,fontSize3,lineH3,align1,title1,fontSize1,lineH2,title2,nextLine,align0,
